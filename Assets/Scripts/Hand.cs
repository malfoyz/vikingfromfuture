...
public class Hand : MonoBehaviour
{
    ...
    void Start()
    {
        point.transform.position = new Vector3(transform.position.x, transform.position.y + 1.2f, transform.position.z);    // точке присваиваем позицию = позиция руки + 1 по ОY
    }
    void Update()
    {
        if (isWait == false)
